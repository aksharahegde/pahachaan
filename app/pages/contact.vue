...
const { data: doc } = await useAsyncData("contact-doc", () =>
  queryCollection("content").where("path", "==", "/contact").first()
);
...
const { data: tree } = await useAsyncData("contact-tree", () => parseMarkdown(doc.value.rawbody));

const pageMeta = computed(() => ({
  title: "Contact",
  description: "How to reach Akshara Hegde.",
  ...(doc.value || {}),
}));
