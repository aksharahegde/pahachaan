...
- Portfolio projects, certifications, and photos are defined as JSON under `content/projects/`, `content/certifications/`, and `content/photos/` respectively; list ordering often relies on numeric filename prefixes.
...
- Visitor globe markers and live presence are Vercel-safe through Ably (`useAblyVisitors().locations`); keep `ABLY_API_KEY` server-only and avoid reintroducing app-hosted Nitro WebSocket visitor routes on Vercel.
- Blog posts and the Uses page render markdown via `@comark/nuxt` (Comark); `@nuxt/content` remains for collections, `queryCollection()`, RSS, and sitemap.
